...
# test that the nesting relation for reports - as the default for that syntax and when declared
#
...
[nng:Report]{ <http://example.org/s>  <http://example.org/p> <http://example.org/o> . }
EOF
...
fgrep -c reports /tmp/test.out | fgrep -s "1"
